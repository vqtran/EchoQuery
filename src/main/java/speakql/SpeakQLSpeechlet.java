...
public class SpeakQLSpeechlet implements Speechlet {
  ...
  public SpeechletResponse onIntent(
    ...
    if ("TableAggregateIntent".equals(intentName)) {
      return TableAggregateIntent(intent, session);
      ...
      String speechOutput =
          "With SpeakQL, you can query"
              + " your database for aggreagtes, group bys, and order bys"
              + " For example, you could say,"
              + " How many nights booked did we have in the past week.";
  ...
  private SpeechletResponse getWelcomeResponse() {
    ...
    // text.
    String repromptText = "With History Buff, you can get historical events "
   ...
   * the Calendar representation of that slot value. If the user provides a
   * date, then use that, otherwise use today. The date is in server time, not
   * in the user's time zone. So "today" for the user may actually be tomorrow.
   * 
   ...
   *     user.
   */
  private SpeechletResponse TableAggregateIntent(
   ...
   * with the user.
   * 
   * @param session Object containing session attributes with events list and
   *     index.
   * @return SpeechletResponse object with voice/card response to return to the
   *     user.
   */
  private SpeechletResponse handleNextEventRequest(Session session) {
    ...
    ArrayList<String> events =
        (ArrayList<String>) session.getAttribute(SESSION_TEXT);
    int index = (int) session.getAttribute(SESSION_INDEX);
    String speechOutput = "";
    String cardOutput = "";
    if (events == null) {
      speechOutput = "With History Buff, you can get historical events for "
          + "any day of the year."
          + " For example, you could say today, or August thirtieth."
          + " Now, which day do you want?";
  ...
  /**
   * Download JSON-formatted list of events from Wikipedia, for a defined
   * day/date, and return a String array of the events, with each event
   ...
   * @return String array of events for that date, 1 event per element of the
   *     array
   */
  private ArrayList<String> getJsonEventsFromWikipedia(
    ...
    try {
      ...
      inputStream =
          new InputStreamReader(url.openStream(), Charset.forName("US-ASCII"));
  ...
  /**
   * Parse JSON-formatted list of events/births/deaths from Wikipedia, extract
   * list of events and split the events into a String array of individual
   * events. Run Regex matchers to make the list pretty by adding a comma after
   * the year to add a pause, and by removing a unicode char.
   * 
   * @param text The JSON formatted list of events/births/deaths for a certain
   *     date.
   * @return String array of events for that date, 1 event per element of the
   *     array.
   */
  private ArrayList<String> parseJson(String text) {
    ...
    while (endIndex != -1) {
      ...
      String eventText = (endIndex == -1)
          ? text.substring(startIndex) : text.substring(startIndex, endIndex);
      ...
      // add comma after year so Alexa pauses before continuing with the
      // sentence
